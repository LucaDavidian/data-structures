...
template <typename T, typename F = decltype(&Less<T>), template <typename> class Container = Vector>
class SortedList
{
public:
...
public: 
    SortedList(const F &comparator = &Less<T>) : mComparator(comparator) {}
...
template <typename T, typename F, template <typename> class C>
template <typename U>
void SortedList<T,F,C>::Insert(U &&element)
{
...
// binary search
template <typename T, typename F, template <typename> class C>
typename SortedList<T,F,C>::Iterator SortedList<T,F,C>::Find(const T &key)
{
